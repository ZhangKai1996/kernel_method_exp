...
from dataset.load import iris_data_set, split_data, dec_to_ter
...
def svc(num_iter, samples, ratio, classes_1=None, classes_2=None, method='origin', n_comp=1):
    ...
    for i in range(num_iter):
        ...
        for k in range(10):
            ...
            # print('\t>>>', k, len(t_X), len(t_y), len(e_X), len(e_y), len(test_X), len(test_y), key)
...
def main(data_set, param):
    fig, axes = plt.subplots(nrows=len(param), ncols=1, sharey=True)
    # fig_stat, axes_stat = plt.subplots(nrows=1, ncols=3, figsize=(15, 10), sharey=True)
    for i, (key, values) in enumerate(param.items()):
        ...
        for j, value in enumerate(values):
            ...
            _, mean_acc_3 = svc(num_iter=10, samples=data_set, ratio=0.7,
                                classes_1=[0, 1, ], classes_2=[2, ],
                                method=key, n_comp=value)
            mean_acc_list.append([mean_acc_1, mean_acc_2, mean_acc_3])
...
def run():
    """Iris"""
    param_lda = {'origin': [1, 2, 3, 4], 'lda': [1, ], 'klda': [1, ]}
    param_pca = {'origin': [1, 2, 3, 4], 'pca': [1, 2, 3, 4], 'kpca': [1, 2, 3, 4]}

    # main(iris_data_set, param_pca)
    main(iris_data_set, param_lda)
