...
def main_1(fig, n_rows, n_columns, start, n_components):
    ...
    ax.axhline(y=min(X_pca[reds, 0]), c='red')
    ax.axhline(y=max(X_pca[reds, 0]), c='red')
    ax.scatter(y[blues]+1, X_pca[blues, 0], c="blue", s=20, edgecolor='k')
    ax.axhline(y=min(X_pca[blues, 0]), c='blue')
    ax.axhline(y=max(X_pca[blues, 0]), c='blue')
    ax.scatter(y[greens]+1, X_pca[greens, 0], c="green", s=20, edgecolor='k')
    ax.axhline(y=min(X_pca[greens, 0]), c='green')
    ax.axhline(y=max(X_pca[greens, 0]), c='green')
    ax.set_xticks(list(range(1, 4)))
    ...
    ax.axhline(y=min(X_kpca[reds, 0]), c='red')
    ax.axhline(y=max(X_kpca[reds, 0]), c='red')
    ax.scatter(y[blues]+1, X_kpca[blues, 0], c="blue", s=20, edgecolor='k')
    ax.axhline(y=min(X_kpca[blues, 0]), c='blue')
    ax.axhline(y=max(X_kpca[blues, 0]), c='blue')
    ax.text(2, -8, '[{:>.2f},{:>.2f}]'.format(min(X_kpca[blues, 0]),
                                              max(X_kpca[blues, 0])), ha='center', va='center')
    ax.scatter(y[greens]+1, X_kpca[greens, 0], c="green", s=20, edgecolor='k')
    ax.axhline(y=min(X_kpca[greens, 0]), c='green')
    ax.axhline(y=max(X_kpca[greens, 0]), c='green')
    ax.text(3, -8, '[{:>.2f},{:>.2f}]'.format(min(X_kpca[greens, 0]),
                                              max(X_kpca[greens, 0])), ha='center', va='center')
    ax.set_xticks(list(range(1, 4)))
...
def main_3(fig, n_rows, n_columns, start, n_components):
    reds = y == 0
    blues = y == 1
    greens = y == 2
...
def run():
    ...
    for n_components_ in range(3):
        ...
        if n_components_ == 0:
            main_1(fig_, 3, 3, n_components_*3, n_components_+1)
        elif n_components_ == 1:
            main_2(fig_, 3, 3, n_components_*3, n_components_+1)
        elif n_components_ == 2:
            main_3(fig_, 3, 3, n_components_*3, n_components_+1)
        else:
            raise NotImplementedError
